class Room {
    ...
    async start() {
        if(this.gameState === 0){
            ...
            this.player1 = createSprite(790, 240);
            this.player2 = createSprite(710, 560);

            this.player1.addImage(player1Image);
            this.player2.addImage(player2Image);

            this.player1.setCollider("circle", 10, 0, 100);
            this.player2.setCollider("circle", -10, 0, 100);
    ...
    play() {
        ...
        for(var i = 0; i <= this.players.length; i++) {
            if(i === player.index){
                ...
                noStroke();
                fill("#ff8a8a");
                ellipse(this.players[i-1].x, this.players[i-1].y, 120, 120);
                pop();
            }
